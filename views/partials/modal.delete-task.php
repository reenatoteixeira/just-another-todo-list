<dialog id="delete-task-modal" class="w-11/12 max-w-lg rounded-lg backdrop:bg-black/60">
  ...
  <form action="/config/process.tasks.php" method="POST">
    ...
    <div class="bg-gray-50 px-4 rounded-b-lg sm:space-x-2">
      <button type="submit" class="mt-3 inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:my-3 sm:w-32">
        Yes, delete
      </button>

      <button type="reset" onclick="closeModal('delete-task-modal')" class="my-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:my-0 sm:w-auto">
        Cancel
      </button>
    </div>
  </form>
