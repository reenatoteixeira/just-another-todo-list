  ...
  <div class="max-w-7xl mx-auto h-full flex items-center justify-between px-4 sm:px-6 lg:px-8">
    <div class="w-full flex items-center justify-between">
      ...
      <div class="hidden flex space-x-2 md:block">
        <?php if ($userData) : ?>
          <a href="/profile" class="<?= checkUrl('/profile') ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' ?> rounded-md px-3 py-2 text-sm font-medium md:text-base">My profile</a>

          <a href="/logout" class="text-gray-300 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium md:text-base">Logout</a>
        <?php else : ?>
  ...
  <div class="hidden md:hidden bg-gray-800 rounded-b-lg" id="mobile-menu">
    ...
    <div class="px-2 pb-3 pt-2 sm:px-3">
      <div class="flex flex-col items-center justify-center sm:flex-row sm:space-y-0 sm:justify-center sm:space-x-2">
        <?php if ($userData) : ?>
          <a href="/profile" class="w-11/12 justify-center rounded-md bg-indigo-600 px-3 py-2 text-sm text-center font-semibold text-white shadow-sm hover:bg-indigo-500 sm:w-2/4">
            <button>My profile</button>
          </a>
          
          <hr class="border-gray-700 w-3/4 my-2 sm:hidden">

          <a href="/logout" class="w-11/12 justify-center rounded-md bg-white px-3 py-2 text-sm text-center font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:w-2/4">
